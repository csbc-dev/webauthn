/**
 * E2E test server for passkey-auth.
 *
 ...
 * authenticator produced — same compromise as s3-uploader's "no SigV4
...
/**
 * Resolve a request path under the package root, refusing anything that
 * escapes it. Mirrors s3-uploader's `staticPath` guard — without it a
...
function safeStaticPath(requestPath: string): string | null {
  ...
  const abs = path.join(PACKAGE_ROOT, normalized.replace(/^\/+/, ""));
  if (!abs.startsWith(PACKAGE_ROOT)) return null;
  return abs;
}
