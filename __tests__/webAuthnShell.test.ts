...
if (!customElements.get("passkey-auth")) {
  customElements.define("passkey-auth", WebAuthn);
}
...
/** Create a ready-to-run `<passkey-auth>` attached to document.body. */
...
describe("<passkey-auth> shell", () => {
  ...
  describe("start() — registration flow", () => {
    it("runs challenge → create() → verify and lands on completed", async () => {
      ...
      el.addEventListener("passkey-auth:status-changed", (e: any) => statuses.push(e.detail));
  ...
  describe("trigger property", () => {
    it("runs start() when set to true and resets to false on completion", async () => {
      ...
      el.addEventListener("passkey-auth:trigger-changed", (e: any) => triggerEvents.push(e.detail));
  ...
  describe("identical-write dedupe on Shell setters (regression — mirrors Core policy)", () => {
    ...
    it("consecutive start() calls on a fresh element do not re-emit identical '' / null state", async () => {
      ...
      el.addEventListener("passkey-auth:credential-id-changed", (e: any) => credIdEvents.push(e.detail));
      el.addEventListener("passkey-auth:user-changed", (e: any) => userEvents.push(e.detail));
      el.addEventListener("passkey-auth:error", (e: any) => errorEvents.push(e.detail));
