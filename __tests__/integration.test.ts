...
if (!customElements.get("passkey-auth-int")) {
  customElements.define("passkey-auth-int", class extends WebAuthn {});
}
 ...
 * the analogue of s3-uploader's "no SigV4 verification — the presigned URL is
...
function mkShell(sessionId: string, attrs: Record<string, string> = {}): WebAuthn {
  const el = document.createElement("passkey-auth-int") as WebAuthn;
