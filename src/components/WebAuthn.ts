 ...
 * to s3-uploader, with credential material taking the place of blob bytes.
 */
export class WebAuthn extends HTMLElement {
  static wcBindable: IWcBindable = {
    ...
    version: 1,
    properties: [
      { name: "status", event: "passkey-auth:status-changed" },
      { name: "credentialId", event: "passkey-auth:credential-id-changed" },
      { name: "user", event: "passkey-auth:user-changed" },
      { name: "error", event: "passkey-auth:error" },
      { name: "trigger", event: "passkey-auth:trigger-changed" },
    ],
    inputs: [
  ...
  set trigger(value: boolean) {
    // Same declarative-trigger contract as <s3-uploader>: flipping trigger true
  ...
  private _setStatus(status: WebAuthnStatus): void {
    ...
    this._status = status;
    this.dispatchEvent(new CustomEvent("passkey-auth:status-changed", {
  ...
  private _setCredentialId(id: string): void {
    ...
    this.dispatchEvent(new CustomEvent("passkey-auth:credential-id-changed", {
  ...
  private _setUser(user: WebAuthnUser | null): void {
    ...
    this._user = user;
    this.dispatchEvent(new CustomEvent("passkey-auth:user-changed", {
  ...
  private _setError(err: Error | null): void {
    ...
    this._error = err;
    this.dispatchEvent(new CustomEvent("passkey-auth:error", {
  ...
  private _setTrigger(v: boolean): void {
    this._trigger = v;
    this.dispatchEvent(new CustomEvent("passkey-auth:trigger-changed", {
